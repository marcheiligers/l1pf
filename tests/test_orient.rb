...
def test_orient_2d(_args, assert)
  assert.equal!(orient([0.1, 0.1], [0.1, 0.1], [0.3, 0.7]), 0)

  assert.true!(orient([0, 0], [-1e-64, 0], [0, 1]) > 0)

  assert.equal!(orient([0, 0], [1e-64, 1e-64], [1, 1]), 0)
  ...
  70.times do |i|
    assert.true!(orient([-x, 0], [0, 1], [x, 0]) > 0)
    assert.equal!(orient([-x, 0], [0, 0], [x, 0]), 0)
    assert.true!(orient([-x, 0], [0, -1], [x, 0]) < 0, "x=#{x}")
    assert.true!(orient([0, 1], [0, 0], [x, x]) < 0, "x=#{x}")
    x *= 10
  end
end
